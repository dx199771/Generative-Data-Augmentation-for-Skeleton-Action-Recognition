...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        #model_output = model_output.permute(0,2,1,3)
        if 'inpainting_mask' in model_kwargs.keys() and 'inpainted_motion' in model_kwargs.keys():
            
            inpainting_mask, inpainted_motion = model_kwargs['inpainting_mask'], model_kwargs['inpainted_motion']
    ...
    def training_losses(self, model, x_start, t, label=None, model_kwargs=None, noise=None, dataset=None, vis_res=False):
        """
        ...
        bs,joint,feat,frame = x_start.shape
        ...
        lenbatch = [frame for b in range(len(x_start))]
        lenbatchTensor = torch.as_tensor(lenbatch)
        databatchTensor = x_start
        mask = lengths_to_mask(lenbatchTensor, databatchTensor.shape[-1]).unsqueeze(1).unsqueeze(1).cuda()
        terms["rot_mse"] = torch.sum(self.masked_l2(target, model_output, mask))
        
        target_vel = (target[:,:,:, 1:] - target[:,:,:, :-1])
        model_output_vel = (model_output[:,:,:, 1:] - model_output[:,:,:, :-1])
        
       
        terms["vel_mse"] = torch.sum(self.masked_l2(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                            model_output_vel[:, :-1, :, :],
                                            mask[:, :, :, 1:]))  # mean_flat((target_vel - model_output_vel) ** 2)
                
        
       
        terms["loss"] = terms["rot_mse"] + terms["cls"] #+ terms["contrastive_loss"]
       
        
        # if vis_res == True:
        #     vis(target.clone().cpu().detach()[:,:75].reshape(target.shape[0],25,3,120)[:,:,:,::2],name="gt")
        #     vis(model_output.clone().cpu().detach()[:,:75].reshape(model_output.shape[0],25,3,120)[:,:,:,::2])
